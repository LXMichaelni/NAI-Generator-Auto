...
class VibeConfigV4 {
  ...
  List<double> availableInformationExtractedValues;
  double selectedInformationExtracted;
  Map<String, String> encodingByInformationExtracted;

  VibeConfigV4({
    required this.fileName,
    required this.vibeB64,
    required this.referenceStrength,
    required this.availableInformationExtractedValues,
    required this.selectedInformationExtracted,
    required this.encodingByInformationExtracted,
    this.imageBytes,
  });
  ...
  factory VibeConfigV4.fromPngBytes(
    ...
    if (extractedVibeB64 != null) {
      return VibeConfigV4(
        ...
        availableInformationExtractedValues: const [1.0],
        selectedInformationExtracted: 1.0,
        encodingByInformationExtracted: {
          normalizeInformationExtracted(1.0): extractedVibeB64
        },
  ...
  factory VibeConfigV4.fromNaiV4VibeJson(
    ...
    // 3. 提取 encodings，并按 information_extracted 建立离散映射。
    final Map<String, String> encodingByInformation = {};
          ...
          for (var typeKey in modelEncodings.keys) {
            ...
            if (typeEncodingInfo != null &&
              ...
              if (encodingValue is String && encodingValue.isNotEmpty) {
                final infoExtracted = _tryParseInformationExtracted(
                    typeEncodingInfo['params'] as Map<String, dynamic>?);
                final normalizedInfo = normalizeInformationExtracted(
                  infoExtracted ?? 1.0,
                );
                encodingByInformation[normalizedInfo] = encodingValue;
              }
            }
          }
        }
    ...
    if (encodingByInformation.isEmpty) {
      throw ArgumentError(
          "Could not find a valid 'encoding' field in the .naiv4vibe JSON data for file '$originalFileName'.");
    }

    final availableInfoExtracted = encodingByInformation.keys
        .map((value) => double.parse(value))
        .toList()
      ..sort();
    final selectedInfoExtracted = availableInfoExtracted.last;
    final selectedEncoding = encodingByInformation[
        normalizeInformationExtracted(selectedInfoExtracted)]!;

    return VibeConfigV4(
      fileName: name, // 使用从JSON中提取的name，或原始文件名
      vibeB64: selectedEncoding,
      referenceStrength: strength.clamp(0.0, 1.0), // 确保强度在有效范围内
      availableInformationExtractedValues: availableInfoExtracted,
      selectedInformationExtracted: selectedInfoExtracted,
      encodingByInformationExtracted: encodingByInformation,
      imageBytes: null, // .naiv4vibe 文件不包含图片预览
    );
  }

  static String normalizeInformationExtracted(double value) {
    final fixed = value.toStringAsFixed(6);
    return fixed.replaceFirst(RegExp(r'\.?0+$'), '');
  }

  static double? _tryParseInformationExtracted(Map<String, dynamic>? params) {
    if (params == null || !params.containsKey('information_extracted')) {
      return null;
    }
    final dynamic value = params['information_extracted'];
